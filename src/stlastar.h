...
public:
  ...
  public:
    Node *parent; // used during the search to record the parent of successor
                  // nodes
    Node *child; // used after the search for the application to view the search
                 // in reverse
...
public:
  virtual ~AStarState() {}
  virtual float GoalDistanceEstimate(T &nodeGoal) = 0; // Heuristic function
                                                       // which computes the
                                                       // estimated cost to the
                                                       // goal node
  virtual bool
  IsGoal(T &nodeGoal) = 0; // Returns true if this node is the goal node
  virtual bool GetSuccessors(AStarSearch<T> *astarsearch,
                             T *parent_node) = 0; // Retrieves all successors to
                                                  // this node and adds them via
                                                  // astarsearch.addSuccessor()
  virtual float GetCost(T &successor) = 0; // Computes the cost of travelling
                                           // from this node to the successor
                                           // node
